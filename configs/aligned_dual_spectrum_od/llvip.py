...
data_root = 'you_llvip_dataset_path'  # 数据集的根目录

model = dict(
    bbox_head=dict(num_classes=1)  # 因为 LLVIP 数据集只有一个类别，所以 num_classes=1
)
...
train_dataloader = dict(
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='coco_annotations/train.json',  # LLVIP 数据集的训练集的注释
        data_prefix=dict(
            _delete_=True,
            img_path_rgb='visible/train/',  # 训练集的可见光 RGB 图像路径
            img_path_ir='infrared/train/'  # 训练集的红外图 IR 像路径
        ),
...
val_dataloader = dict(
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='coco_annotations/test.json',  # LLVIP 数据集的测试集的注释，因为没有 val 部分，所以使用 test.json
        data_prefix=dict(
            _delete_=True,
            img_path_rgb='visible/test/',  # 测试集的可见光 RGB 图像路径
            img_path_ir='infrared/test/'  # 测试集的红外图 IR 像路径
        ),
        test_mode=True,
...
val_evaluator = dict(
    type='CocoMetric',
    ann_file=data_root + '/coco_annotations/test.json',  # LLVIP 数据集的测试集的注释，因为没有 val 部分，所以使用 test.json
    metric='bbox',
    format_only=False,
...
param_scheduler = [
    dict(
        ...
        by_epoch=True,
        milestones=[15, 18],  # 在第15 和 18 个epoch 减小学习率
        gamma=0.1)
]
